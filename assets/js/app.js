...
let liveSocket = new LiveSocket("/live", Socket, {params: {_csrf_token: csrfToken, a: 1, viewport: {
...
window.adjustGridSize = function adjustGridSize() {
...
function checkGridLoaded() {
    console.log("checkGridLoaded");
    if($("[id^=location]").length == 0) {
        adjustGridSize();
        window.setTimeout(checkGridLoaded, 500);
    }
}

window.loadGrid = function loadGrid() {
    if(liveSocket.isConnected() === false) {
        window.setTimeout(loadGrid, 100); /* this checks the flag every 100 milliseconds*/
    } else {
        checkGridLoaded();
    }
}

// window.addEventListener('dim_initialized', adjustGridSize());
