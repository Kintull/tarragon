defmodule TarragonWeb.PageLive.Ecspanse.Dashboard.NavData do
  @moduledoc """
  ...
  * selected_sub_tab_parent_tab - the main tab the selected_sub_tab is from.
  ...
  def clear_selected_sub_task_if_not_from(%__MODULE__{} = nav_data, tab) do
    if is_selected_sub_task_from_tab?(nav_data, tab) do
      nav_data
    else
      Map.put(nav_data, :selected_sub_tab, nil)
    end
  end

  def is_selected_sub_task_from_tab?(%__MODULE__{} = nav_data, tab),
    do: nav_data.selected_sub_tab_parent_tab == tab
  ...
  def set_selected_tab(
        %__MODULE__{} = nav_data,
        selected_tab,
        clear_selected_sub_tab \\ false
      ) do
    selected_sub_tab =
      if clear_selected_sub_tab do
        nil
      else
        nav_data.selected_sub_tab
      end

    struct!(nav_data, selected_tab: selected_tab, selected_sub_tab: selected_sub_tab)
  end
